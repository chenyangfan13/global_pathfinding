...
namespace global_planner {
    ...
    bool GlobalPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,  std::vector<geometry_msgs::PoseStamped>& plan ){
        ...
        std::map<Cell, int> f_value;             
        f_value[src] = heuristic_calc(goal_cell, src , costmap_ros_);
                ...
                if(isValid(neigh)){
                    if(visited.find(neigh) == visited.end()){                            //i.e new cell found 
                        ...
                        if(curr_cell.dist + neigh_cost  < dist[neigh] ){
                            ...
                            //f_value[neigh] = neigh.dist + heuristic_calc(goal_cell, neigh , costmap_ros_) ;
                            neigh.f_value = neigh.dist + 1000*heuristic_calc(goal_cell, neigh , costmap_ros_) ;
                            //测试g与h数量级关系
                            //cout<<neigh.dist<<"     "<<heuristic_calc(goal_cell, neigh , costmap_ros_)<<endl;
                            visited[neigh] = true;
